<template>
    <div class="w-full">
        <form @submit.prevent="simpanDistribusi" class="flex flex-col gap-6">
            ...
            <!-- HEADER FORM (Tanpa Dropdown Perusahaan) -->
            ...
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm mt-2 relative animate-fade-in">
                ...
                <!-- Loading State saat menarik seluruh stok dari backend -->
                <div v-if="sedangMuatStok" class="absolute inset-0 bg-white/70 backdrop-blur-sm z-10 flex items-center justify-center rounded-xl">
                    <div class="flex flex-col items-center gap-2">
                        ...
                        <span class="text-xs font-bold text-slate-500">Memuat Stok Pabrik...</span>
                    </div>
                </div>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr v-for="(item, index) in form.items" :key="index" class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="p-3 align-top">
                                    <select v-model="item.stiker" @change="resetBarang(item)" required
                                        class="w-full bg-white border border-slate-300 rounded-sm px-3 py-2 text-sm font-semibold text-blue-700 focus:ring-1 focus:ring-blue-500 focus:outline-none cursor-pointer transition-all">
                                ...
                                <td class="p-3 align-top">
                                    <!-- Menggunakan getBarangTersedia(item) dari utility baru -->
                                    <select v-model="item.barang_nama" @change="resetKemasan(item)" :disabled="!item.stiker" required 
                                        ...
                                        <option v-for="nama in getBarangTersedia(item)" :key="nama" :value="nama">{{ nama }}</option>
                                    </select>
                                ...
                                <td class="p-3 align-top">
                                    <!-- Menggunakan getKemasanTersedia(item) dari utility baru -->
                                    <select v-model="item.stok_terpilih" @change="hitungOtomatis(item)" :disabled="!item.barang_nama" required 
                                        ...
                                        <option v-for="(kemasan, idx) in getKemasanTersedia(item)" :key="idx" :value="kemasan">
                                ...
                                <td class="p-3 align-top">
                                    <input type="number" v-model="item.total_unit" @input="kalkulasiBerat(item)" min="1" :disabled="!item.stok_terpilih" required 
                ...
                <div class="p-4 bg-slate-50 border-t border-slate-200 text-center rounded-b-xl">
                    <button type="button" @click="tambahItem" class="text-sm font-bold text-blue-600 hover:text-blue-700 flex items-center justify-center gap-2 mx-auto px-4 py-2 hover:bg-blue-50 rounded-sm transition-colors">
            ...
            <div class="flex justify-end mt-4">
                <button type="submit" :disabled="sedangProses || form.items.length === 0" class="px-8 py-3 bg-slate-900 text-white font-bold hover:bg-slate-800 disabled:bg-slate-400 transition-colors rounded-sm shadow-md flex items-center gap-2">
...
const {
    sedangProses, sedangMuatStok, daftarToko, daftarArmada, 
    form, previewNomor, daftarGrupUnik, getBarangTersedia, 
...
// Tarik data saat halaman pertama kali dirender
